  ...
  useEffect(() => {
    if (screenId === 36) {
      const fetchDashboardData = async () => {
        try {
          setLoading(true);
          const [summary, cases, priorities] = await Promise.all([
            getDashboardSummary(),
            getRecentCases(),
            getPriorityCases()
          ]);
              ...
              <div className="xl:col-span-2 space-y-6">
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    {(priorityCases.length > 0 ? priorityCases.slice(0, 4) : CRITICAL_QUEUE.slice(0, 4)).map((item, i) => (
                      <button
                        key={i}
                        onClick={() => navigate(45)}
                      ...
                      >
                        {/* Color Bar */}
                        <div className={`h-1.5 w-full ${item.acuity === 'CRITICAL' ? 'bg-gradient-to-r from-red-500 to-red-600' :
                        ...
                        <div className="p-5 flex gap-4">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2 mb-3">
                              <span className={`text-xs font-bold px-3 py-1 rounded-full uppercase ${item.acuity === 'CRITICAL' ? 'bg-red-100 text-red-700' :
            ...
            <div className="w-full max-w-xs bg-white rounded-3xl shadow-xl border border-slate-100 overflow-hidden relative aspect-square group">
              ...
              <div className="absolute bottom-0 w-full bg-white/90 backdrop-blur-md p-4 border-t border-slate-100">
                <div className="flex items-center gap-3">
                  <div className={`w-3 h-3 rounded-full ${scanStatus === 'Not Scanned' ? 'bg-slate-300' :
